        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="glass-card rounded-xl p-5 border border-zinc-800/80 space-y-4">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[11px] font-mono text-zinc-400">@averyfoundit</span>
            </div>

            {/* Connected Threads Profile Avatar & Bio Card */}
            <div className="flex items-center gap-4 p-3 bg-zinc-900/70 rounded-xl border border-zinc-800/80">
              <div className="relative shrink-0">
                <img
                  src={persona.avatarUrl || '/avatar.jpg'}
                  alt={persona.identityName}
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <span className="font-semibold text-sm text-white truncate">{persona.identityName}</span>
                  <span className="text-xs text-zinc-400 font-mono">@averyfoundit</span>
                  <span className="px-2 py-0.5 rounded text-[10px] bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 font-medium">
                    {language === 'id' ? 'Akun Threads Aktif' : 'Active Threads Account'}
                  </span>
                </div>
                <p className="text-[11px] text-zinc-400 truncate mt-0.5">{persona.tagline}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="md:col-span-2">
                <label className="text-xs text-zinc-400 block mb-1">
                  {language === 'id' ? 'URL Foto Profil Avatar' : 'Avatar Profile Picture URL'}
                </label>
                <input
                  type="text"
                  value={persona.avatarUrl || ''}
                  onChange={(e) => setPersona({ ...persona, avatarUrl: e.target.value })}
                  placeholder="https://res.cloudinary.com/... atau /avatar.jpg"
                  className="w-full bg-zinc-900 border border-zinc-800 rounded-lg px-3 py-2 text-xs text-white font-mono focus:outline-none focus:border-zinc-600"
                />
              </div>
            </div>
          </div>
